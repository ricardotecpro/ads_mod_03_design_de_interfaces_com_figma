...
def test_homepage_structure(page: Page, base_url):
    ...
    # Check title
    expect(page).to_have_title(re.compile(r"Design de Interfaces com Figma"))
    
    # Check main heading
    heading = page.locator("h1")
    expect(heading).to_contain_text("Design de Interfaces com Figma")
...
def test_quiz_functionality(page: Page, base_url):
    ...
    if first_quiz.is_visible():
        ...
        if correct_option.count() > 0:
            correct_option.first.click()
...
# Test 6: Lesson 16 page (Hand-off)
